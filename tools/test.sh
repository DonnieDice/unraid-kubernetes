...
test "$(xmllint --xpath 'string(/Plugin/Support)' plugins/unraid-kubernetes.xml)" = 'https://github.com/DonnieDice/unraid-kubernetes/issues'
test "$(xmllint --xpath 'count(/CommunityApplications/Forum)' ca_profile.xml)" = '0'
grep -q 'https://github.com/DonnieDice/unraid-kubernetes/issues' src/usr/local/emhttp/plugins/unraid.kubernetes/KubernetesSettings.page
grep -Eq '^[0-9]{4}\.[0-9]{2}\.[0-9]{2}[a-z]?$' VERSION
